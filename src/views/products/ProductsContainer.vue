<template>
    ...
    <div v-if="productsStatus.loading" class="flex justify-center items-center h-full">
        <Loading stroke-color="#8f8f8f" />
    </div>
    <div v-if="productsStatus.error" class="flex justify-center items-center h-full">
        <span class="text-red-500 text-center h-5 ">
            <span>{{ productsStatus.message }}</span>
        </span>
    </div>
    <div v-if="productsStatus.success && !products.length" class="flex justify-center items-center h-full">
        <span class="text-gray-500 text-center h-5 ">
            <span>لا يوجد منتجات</span>
        </span>
    </div>
    <ProductsList v-if="productsStatus.success && products.length" :products="products" :openUpdateDialog="openUpdateDialog"
        :openDeleteDialog="openDeleteDialog" />
    <PaginationItems v-if="!productsStatus.error && products.length" :currentPage="pagination.currentPage"
        :totalPages="pagination.lastPage" :goToPage="(page: number) => { productsStore.fetchProducts(page) }" />
    ...
    <EmptyDialog v-if="updatePopup" title="تعديل" :onClose="closeUpdatePopup" :closeDialog="closeUpdatePopup">
        <UpdateProduct :update="updateProduct" :status="productStatus" :product="product" />
    </EmptyDialog>
    <TrueOrFalseDialog v-if="deletePopup" title="حذف" :onClose="closeDeletePopup" :closeDialog="closeDeletePopup"
        :message="'هل تريد حذف المنتج ' + product.name + '؟'" :trueText="'نعم'" :falseText="'لا'"
        :selectOption="handleDelete" :status="productStatus">
    </TrueOrFalseDialog>
</template>
