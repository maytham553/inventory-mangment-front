    ...
    <form @submit.prevent="handleSubmit" class="w-full h-full flex justify-center flex-col items-center">
        <div class="bg-gray-100 p-5 rounded-2xl">
            <div class="mb-4">
                ...
                <input type="string" id="name" v-model="formData.name" required class="form-input w-full p-1 rounded-lg" />
            </div>
            <div class="mb-4">
                <label for="cost" class="block text-gray-700 m-1">التكلفة</label>
                <input type="number" id="cost" v-model="formData.cost" required class="form-input w-full p-1 rounded-lg" />
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="string" id="barcode" v-model="formData.barcode" required
                    class="form-input w-full p-1 rounded-lg" />
            </div>
            <div class="mb-4">
                <label for="code" class="block text-gray-700 m-1">الكود</label>
                <input type="string" id="code" v-model="formData.code" required class="form-input w-full p-1 rounded-lg" />
            </div>
            <div class="mb-4">
...
import type { Status, RawMaterial } from '../../Types';;
...
const formData = props.rawMaterial ? ref({ ...props.rawMaterial }) : ref({} as RawMaterial);

const handleSubmit = async () => {
    await props.onSubmit(formData.value);
    formData.value = { ...props.rawMaterial } as RawMaterial;
};
</script>