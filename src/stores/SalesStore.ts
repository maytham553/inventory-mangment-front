  ...
  actions: {
    async fetchSalesOfCustomer(page = 1, id: number, search = "") {
      ...
      try {
        ...
        const { data: response } = await customers.getCustomerSales(
          id,
          page,
          search
        );
    ...
    addProductToSale(product: Product) {
      const id = product.id;
      const productIndex = this.sale.products.findIndex(
    ...
    reCalculateSaleAfterChange() {
      ...
      if (this.sale.products && this.sale.products.length > 0) {
        this.sale.products.forEach((product: SaleProduct) => {
          ...
          if (discountAmount) {
            product.discount_percentage = calculateDiscountPercentage(
              product.subtotal,
              discountAmount,
              1
            );
          ...
          if (product.quantity) {
            product.subtotal = calculateSubtotal(
              product.quantity,
              product.unit_price,
              1
            );
        ...
        if (this.sale.discount_amount) {
          this.sale.discount_percentage = calculateDiscountPercentage(
            this.sale.subtotal_amount,
            this.sale.discount_amount,
            1
          );
